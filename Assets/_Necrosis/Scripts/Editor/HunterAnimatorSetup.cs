...
public static class HunterAnimatorSetup
{
    ...
    {
        HunterDir + "/zombie.fbx",
        HunterDir + "/parasite.fbx",
        HunterDir + "/zombie_girl.fbx",
    };
    ...
    static readonly (string file, bool loop)[] Anims =
    {
        ...
        (AnimDir + "/scream.fbx",        true),
        (AnimDir + "/attack.fbx",        false),
    ...
    static void BuildController()
    {
        ...
        tree.AddChild(LoadClip(AnimDir + "/walk_straight.fbx"), 1f);
        tree.AddChild(LoadClip(AnimDir + "/run_straight.fbx"), 2f); // la persecución baja a ~2.3 con poca luz; patrulla ~1.6 se queda en walk
        sm.defaultState = loco;

        // Ataque: desde cualquier estado cuando Attacking; vuelve al soltarlo
        var attack = sm.AddState("Attack");
        attack.motion = LoadClip(AnimDir + "/attack.fbx");
